		...
		zbx_hashset_t *itemids)
{
	...
	zbx_snprintf_alloc(&sql, &sql_alloc, &sql_offset,
			" from items t,hosts r where t.hostid=r.hostid"
				...
				" and r.status in (%d,%d)"
				" and t.flags<>%d"	/* RSM specifics: move all preprocessing to server */
...
int	get_proxyconfig_data(zbx_uint64_t proxy_hostid, struct zbx_json *j, char **error)
{
	static const char	*proxytable[] =
	{
		...
		"items",
		"item_rtdata",
		/*"item_preproc", RSM specifics: move all preprocessing to server */
		"item_parameter",
		"drules",
		"dchecks",
		"regexps",
		...
		zbx_proxy_suppress_t *nodata_win, char **info, unsigned int mode)
{
			...
			&unique_shift, &error) && 0 != values_num)
	{
		...
		for (i = 0; i < values_num; i++)
		{
			...
			/* RSM specifics: ignore history data from proxy if it's older than 90 seconds */
			if (0 != items[i].host.proxy_hostid && 90 < now - values[i].ts.sec && (
