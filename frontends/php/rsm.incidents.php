...
if (isset($_REQUEST['favobj'])) {
	if('filter' == $_REQUEST['favobj']) {
		CProfile::update('web.rsm.incidents.filter.state', getRequest('favstate'), PROFILE_TYPE_INT);
	}
}
...
if (isset($_REQUEST['mark_incident']) && (CWebUser::getType() == USER_TYPE_ZABBIX_ADMIN
	...
	if (getRequest('mark_incident') == INCIDENT_ACTIVE || getRequest('mark_incident') == INCIDENT_RESOLVED) {
...
if (getRequest('filter_rolling_week')) {
	...
	if (getRequest('filter_set')) {
		if (getRequest('filter_from') == getRequest('original_from')) {
			$data['filter_from'] = date('YmdHis', getRequest('filter_from', $serverTime - $rollWeekSeconds['value']));
		}
		else {
			$data['filter_from'] = getRequest('filter_from', date('YmdHis', $serverTime - $rollWeekSeconds['value']));
		}
		if (getRequest('filter_to') == getRequest('original_to')) {
			$data['filter_to'] = date('YmdHis', getRequest('filter_to', $serverTime));
		}
		else {
			$data['filter_to'] = getRequest('filter_to', date('YmdHis', $serverTime));
		}
		CProfile::update('web.rsm.incidents.filter_from', $data['filter_from'], PROFILE_TYPE_ID);
		CProfile::update('web.rsm.incidents.filter_to', $data['filter_to'], PROFILE_TYPE_ID);
	}
...
$data['type'] = getRequest('type', get_cookie('ui-tabs-1', 0));
